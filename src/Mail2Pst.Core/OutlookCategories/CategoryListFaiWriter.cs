...
/// <c>PidTagRoamingXmlStream</c> (MS-OXOCFG §2.2.5.1.1). Classic Outlook reads category colours from
/// this FAI on every attached store, so a converted PST opened as a secondary store renders its
/// categories in colour with no Outlook automation. Idempotent: a second stamp updates the existing
